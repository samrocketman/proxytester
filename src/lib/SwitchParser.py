...
class SwitchParser:
	...
	"""
		...
		-f filter by two letter country codes (ISO_3166), requires ipdb.txt from site
	"""
	SWITCHES = ["E","GET","MT","O","Q","RESPONSE","T","U","W", "F"]
				...
				if self.SWITCHES.count(switch) > 0 :
					if self.SWITCHES[0] == switch : #The next argument is a file
						...
						if not os.path.isfile(self.excludeFile) :
							print "Exclude file (-E) must exist!"
							self.syntaxErr()
					...
					if self.SWITCHES[2] == switch : # Threading
						self.Threads=4
					if self.SWITCHES[3] == switch : #The next argument is a file
						try:
							...
							print "Possible missing argument."
							self.syntaxErr()
					if self.SWITCHES[4] == switch :	 # Quiet mode
						self.quietMode = True
					...
					if self.SWITCHES[7] == switch : # Uniques
						self.unique = True
					if self.SWITCHES[8] == switch : # WPAD filter
						self.WPAD = True
					if self.SWITCHES[9] == switch : # Country filter
						self.countryFilter = True	# Test for ipdb.txt existing, get country code
						try:											# First check for switch args
							argument = arguments[i+1]
						except:
							print "Missing argument."
							self.syntaxErr()
							
						if not os.path.isfile(self.ipdb) : # check for ipdb
								print "ipdb.txt must exist!"
								self.syntaxErr()
						f = open(self.ipdb)
						fileContents = f.read()
						fileContents = fileContents.strip('\r')
						...
						p=re.compile(self.COUNTRY_CODE)
						if p.match(argument) == None :
							print "Country code must be two letters."
						self.countryCode = argument
						# 																TODO: get code from cmd line and send to main
						#	
				else :
					print "Invalid argument!"
					self.syntaxErr()
			else :
				if argument != self.outFile and argument != self.restrictedURL and argument != str(self.Timeout) and argument != self.responseFile and argument != self.excludeFile and argument != self.countryCode:
	...
	def showhelp(self) :
		...
		exit()
	def syntaxErr(self) :
		print "The syntax is incorrect.	Try proxytester.py /? or proxytester.py --license."
		exit()
